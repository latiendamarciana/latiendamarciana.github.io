---
# --- Identificación ---
layout: responses
title: "La Tienda Marciana suscripción confirmada"
name: "Suscripción confirmada"
permalink: /suscripcion-confirmada/

# --- SEO / Metadatos ---
description: "La Tienda Marciana, página para informar que la suscripción al boletín de correo está confirmada"
author: "fluorescente.design"
date: 2026-07-30
tags: ["Tienda", "Obra gráfica", "Comida a base de plantas", "Accesorios", "Música", "Medellín", "Colombia", "Comida vegana", "Marciana"]
keywords: "Tienda, Obra gráfica, Comida a base de plantas, Accesorios, Música, Medellín, Colombia, Comida vegana, Marciana"
image: "assets/images/basic/estilos-y-componentes-basicos-card.png"

# --- Contenido ---
src: "/assets/images/respuestas/marciana-suscripcion-confirmada-transparente-lg.png"
image-xs: "/assets/images/respuestas/marciana-suscripcion-confirmada-transparente-xs.png"
image-sm: "/assets/images/respuestas/marciana-suscripcion-confirmada-transparente-sm.png"
image-lg: "/assets/images/respuestas/marciana-suscripcion-confirmada-transparente-lg.png"
alt: "Marciana junto a la puerta abierta de un local, invitando a entrar"
heading: "¡Suscripción confirmada!"
button-text: "Volver"
href: "/"
---

Ya estás adentro. A partir de ahora recibirás en tu correo:

* Noticias sobre ferias y eventos en los que participaré
* Si consigo food truck, o local te avisaré para que caigas
* Lanzamientos de obra gráfica
* Colaboraciones y sorpresas

Usaré tu correo únicamente para esto, no será compartido con nadie, y podrás cancelar tu suscripción en cualquier momento con solo unos clics.